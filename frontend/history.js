...
class HistoryPageManager {
    ...
    async loadHistoryPage() {
        try {
            ...
            const raw = await getPlayHistory(this.currentPage, this.pageSize, this.currentFilter);
            const records = compactHistoryRecords(raw && raw.records ? raw.records : []);
            const historyData = { records, total_count: records.length };
...
function createSongItemElement(record, index) {
    ...
    const rawCover = record.union_cover || '';
    const coverUrl = rawCover
        ? getCoverImageUrl(rawCover.includes('{size}') ? rawCover : rawCover.replace('/120/', '/{size}/'), 150)
        : '';

    const recordDate = parseHistoryDate(record);
    const playTime = recordDate
        ? recordDate.toLocaleTimeString('zh-CN', { hour: '2-digit', minute: '2-digit' })
        : '--:--';

    const duration = formatDuration(record.time_length);
    const playCount = record.play_count || 1;
...
function compactHistoryRecords(records) {
    ...
    for (const r of records || []) {
        const hash = String(r.hash || '').toUpperCase();
        if (!hash) continue;
        const prev = map.get(hash);
        const count = Number(r.play_count) > 0 ? Number(r.play_count) : 1;
        if (!prev) {
            map.set(hash, { ...r, hash, play_count: count });
        } else {
            prev.play_count = (Number(prev.play_count) || 1) + count;
            const pt = parseHistoryDate(r);
            const ppt = parseHistoryDate(prev);
            if (pt && (!ppt || pt > ppt)) {
                prev.play_time = r.play_time ?? r.last_play_time ?? prev.play_time;
                prev.last_play_time = prev.play_time;
                for (const k of ['songname', 'filename', 'author_name', 'album_name', 'album_id', 'union_cover', 'time_length']) {
                    if (r[k]) prev[k] = r[k];
                }
            }
        }
...
function parseHistoryDate(record) {
    const t = record.play_time ?? record.last_play_time;
...
function groupRecordsByDate(records) {
    ...
    records.forEach(record => {
        ...
        if (!recordDate) {
            dateKey = '更早';
        } else if (isSameDay(recordDate, today)) {
            dateKey = '今天';
