...
async function handleClaimVip() {
    ...
    try {
        ...
        const now = new Date();
        const receiveDay = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}-${String(now.getDate()).padStart(2, '0')}`;
        const response = await LoginService.ClaimDailyVip(receiveDay);
        ...
        if (response.success) {
            ...
            claimVipBtn.title = response.message || '领取失败';
...
async function checkLoginStatusOnStartup() {
    ...
    try {
        ...
        if (response.success) {
            ...
            isLoggedIn = true;
            userInfo = {
                loginMethod: info.login_method || 'unknown',
                userid: response.data?.userid,
                userData: {
                    ...
                    login_method: info.login_method || 'unknown',
                    vip_detail: info.vip_detail
                },
                loginTime: info.login_time ? new Date(info.login_time * 1000).toISOString() : new Date().toISOString()
            };
