...
class AlbumDetailManager {
    ...
    isAlbumDetailActive() {
        const page = document.getElementById('albumDetailPage');
    ...
    updateLoadMoreButton() {
        ...
        // 离开专辑/歌单页后不往 body 挂固定按钮，避免飘到其它页
        if (!this.isAlbumDetailActive()) {
            return;
        }
        ...
        if (this.currentType === 'playlist' && this.hasMoreData) {
            ...
            // 挂到专辑详情页内：页隐藏时按钮一起隐藏；fixed 仍相对视口
            const albumDetailPage = document.getElementById('albumDetailPage');
            albumDetailPage.insertAdjacentHTML('beforeend', buttonHTML);
        }
    }
