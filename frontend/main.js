...
window.maximizeWindow = () => {
    Window.Maximise();
    setTimeout(() => {
        if (window.updateMaximizeIcon) window.updateMaximizeIcon();
    }, 80);
}
...
document.addEventListener('click', (e) => {
    ...
    if (!el) return;
    const btn = el.closest('.titlebar-btn');
    if (!btn || !btn.closest('.window-controls')) return;
    e.preventDefault();
    e.stopPropagation();
...
function showMainContent(pageState) {
    // 离开专辑/歌单页时卸掉右下角「加载更多」，避免固定层飘到其它页
    if (pageState !== PAGE_STATES.ALBUM_DETAIL && window.AlbumDetailManager) {
        window.AlbumDetailManager.clearLoadMoreButton();
    }
...
function isTitlebarDragTarget(el) {
    return !el.closest('button, input, a, select, textarea, .window-controls, .search-container, .user-controls, .titlebar-nav-buttons');
}
...
function setupTitlebarDrag() {
    ...
    titlebar.addEventListener('mousedown', (e) => {
        ...
        if (!isTitlebarDragTarget(e.target)) return;
    ...
    window.addEventListener('mousemove', (e) => {
        ...
        fetch('/__window', {
            ...
            body: JSON.stringify({ op: 'drag', x, y }),
        });
    });
