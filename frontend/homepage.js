...
async function getSongPlayUrls(hash) {
    try {
        ...
        try {
            const cacheResponse = await GetCachedURL(hash);
            if (cacheResponse.success && cacheResponse.data) {
                console.log('🎵 使用本地音频缓存:', cacheResponse.data);
                HomepageService.GetSongUrl(hash).then((response) => {
                    if (response?.success && response.data?.lyrics) {
                        currentSongLyrics = response.data.lyrics;
                        window.currentSongLyrics = response.data.lyrics;
                        updateLyricsDisplay(response.data.lyrics);
                    }
                }).catch(() => {});
                return [cacheResponse.data];
            }
        } catch (cacheErr) {
            console.warn('⚠️ 读取音频缓存失败，走在线地址:', cacheErr);
        }
...
window.handleFmSongEnded = handleFmSongEnded;
window.preloadMoreFmSongs = preloadMoreFmSongs;
window.extendEndlessPlaylist = extendEndlessPlaylist;
window.isFmPlaying = isFmPlaying;
window.isAiPlaying = isAiPlaying;
// 暴露首页刷新函数
    ...
    if (fmPlayOverlay) {
        fmPlayOverlay.addEventListener('click', async (e) => {
            ...
            if (currentFmSong && window.PlayerController) {
                ...
                const idx = Math.max(0, fmPlaylist.findIndex(s => s.hash === currentFmSong.hash));
                const success = await window.PlayerController.playPlaylist(fmPlaylist, idx, '私人FM', 'list');
                if (success) {
    ...
    if (aiPlayOverlay) {
        aiPlayOverlay.addEventListener('click', async (e) => {
            ...
            if (currentAiSong && window.PlayerController) {
                ...
                const idx = Math.max(0, currentAiRecommendList.findIndex(s => s.hash === currentAiSong.hash));
                const success = await window.PlayerController.playPlaylist(currentAiRecommendList, idx, 'AI推荐', 'list');
                if (success) {
...
function mapHomeSongs(list) {
    return (list || []).map(songData => ({
        hash: songData.hash,
...
async function preloadMoreFmSongs(force = false) {
    ...
    try {
        ...
        const have = new Set(fmPlaylist.map(s => s.hash));
        const added = mapHomeSongs(response.data).filter(s => !have.has(s.hash));
        if (added.length === 0) return;
        fmPlaylist = [...fmPlaylist, ...added];
        const pl = window.PlaylistManager ? window.PlaylistManager.getCurrentPlaylist() : null;
        const idx = pl?.current_index ?? pl?.CurrentIndex ?? fmCurrentIndex;
        fmCurrentIndex = idx;
        if (window.PlayerController && window.PlayerController.updatePlaylist) {
            await window.PlayerController.updatePlaylist(fmPlaylist, idx, '私人FM');
        }
    } catch (error) {
...
async function appendMoreAiSongs() {
    try {
        const response = await HomepageService.GetAIRecommend();
        if (!(response.success && response.data && response.data.length > 0)) {
            console.warn('追加AI推荐无数据', response);
            return;
        }
        const have = new Set(currentAiRecommendList.map(s => s.hash));
        const added = mapHomeSongs(response.data).filter(s => !have.has(s.hash));
        if (added.length === 0) return;
        currentAiRecommendList = [...currentAiRecommendList, ...added];
        const pl = window.PlaylistManager ? window.PlaylistManager.getCurrentPlaylist() : null;
        const idx = pl?.current_index ?? pl?.CurrentIndex ?? 0;
        if (window.PlayerController && window.PlayerController.updatePlaylist) {
            await window.PlayerController.updatePlaylist(currentAiRecommendList, idx, 'AI推荐');
        }
    } catch (error) {
        console.error('追加AI推荐失败:', error);
    }
}
...
async function likeFmSong() {
    const song = (isFmPlaying() && window.PlayerController?.getCurrentSong()) || currentFmSong;
    if (!song) return;
...
async function dislikeFmSong() {
    ...
    try {
        if (song.hash) {
            await HomepageService.ReportFMAction(song.hash, '', 'garbage', 0);
        }
    } catch (error) {
        console.error('不喜欢操作API调用失败:', error);
    }
    if (isFmPlaying() && window.PlayerController?.playNext) {
        await window.PlayerController.playNext();
        currentFmSong = window.PlayerController.getCurrentSong() || currentFmSong;
        updateFmDisplay();
    } else {
        await nextFmSong();
    }
...
async function likeAiSong() {
    const song = (isAiPlaying() && window.PlayerController?.getCurrentSong()) || currentAiSong;
    if (!song) return;
    await addToFavorites(song);
}
...
async function dislikeAiSong() {
    ...
    if (isAiPlaying() && window.PlayerController?.playNext) {
        await window.PlayerController.playNext();
        currentAiSong = window.PlayerController.getCurrentSong() || currentAiSong;
        updateAiDisplay();
    } else {
        await nextAiSong();
    }
