...
class ImmersivePlayer {
    ...
    updateBackground(coverUrl) {
        ...
        const backgroundImageUrl = window.getCoverImageUrl ? window.getCoverImageUrl(coverUrl, 200) : coverUrl;
        if (this.lastBackgroundUrl === backgroundImageUrl) return;
        this.lastBackgroundUrl = backgroundImageUrl;
    ...
    setupMouseActivityDetection() {
        ...
        const resetTimer = () => {
            const now = Date.now();
            if (now - (this._lastMouseReset || 0) < 200) return;
            this._lastMouseReset = now;
            ...
            if (isMouseInside) {
                mouseTimer = setTimeout(() => {
                    if (isMouseInside) hideControls();
                }, 3000);
                cursorTimer = setTimeout(() => {
                    if (isMouseInside) hideCursor();
                }, 2000);
            }
        };
        ...
        this.container.addEventListener('mouseenter', () => {
            isMouseInside = true;
            this._lastMouseReset = 0;
            resetTimer();
        });

        this.container.addEventListener('mousemove', () => {
            if (isMouseInside) resetTimer();
        });
        // 鼠标离开容器
...
if (window.addEventListener) {
    ...
    const observeLyricsChanges = () => {
        ...
        if (mainLyricsDisplay) {
            ...
            addObserver(mainLyricsDisplay, (mutations) => {
                if (!immersivePlayer.isActive) return;
                ...
                if (contentChanged) {
                    clearTimeout(immersivePlayer._lyricsSyncTimer);
                    immersivePlayer._lyricsSyncTimer = setTimeout(() => {
                        immersivePlayer.syncLyrics();
                    }, 200);
                }
            }, {
